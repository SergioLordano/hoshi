...
#########################################################################
#### imputs
filename = '/media/lordano/DATA/Mestrado/samples/siemens_penelope.png'
...
transmission_filename = '/media/lordano/DATA/Mestrado/siemens_PEN_T{0}nm_W{1}nm_transm_E_{2}eV.txt'.format(int(max_thickness*1e9), int(img_width*1e9), int(energy))

#########################################################################


#### import image and normalize to thickness
img = hsrw.image_to_thickness(filename, max_thickness=max_thickness, rgb_channel=0, invert=False)
...
if(1):
    ...
    fig, ax = plt.subplots(figsize=(8.0, 5.5), nrows=2, ncols=2, sharex=True, sharey=True)
    plt.subplots_adjust(0.15, 0.15, 0.85, 0.9, hspace=0.2)
    
    ax[0,0].set_title('thickness')
    im00 = ax[0,0].imshow(thickness*1e9, cmap=cmap, extent=ext)
    fig.colorbar(im00, ax=ax[0,0], label='nm')
    ax[0,0].set_ylabel('y [um]')
    plt.locator_params(axis='x', nbins=3)
    plt.locator_params(axis='y', nbins=3)
    
    ax[0,1].set_title('OPD')
    im12 = ax[0,1].imshow(opd*1e12, cmap=cmap, extent=ext)
    fig.colorbar(im12, ax=ax[0,1], label='pm')
    
    
    ax[1,0].set_title('amplitude')
    im10 = ax[1,0].imshow(amplitude, cmap=cmap, extent=ext)
    fig.colorbar(im10, ax=ax[1,0])
    ax[1,0].set_ylabel('y [um]')
    ax[1,0].set_xlabel('x [um]')
    ...
    ax[1,1].set_title('phase')
    im11 = ax[1,1].imshow(phase, cmap=cmap, extent=ext)
    fig.colorbar(im11, ax=ax[1,1], label='rad')
    ax[1,1].set_xlabel('x [um]')
